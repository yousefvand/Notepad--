...
#include "interpret_as_windows_1254.h"
#include <QFile>
#include <QDebug>

// Singleton instance
Interpret_As_windows_1254& Interpret_As_windows_1254::instance() {
    static Interpret_As_windows_1254 instance;
    return instance;
}

// Constructor
Interpret_As_windows_1254::Interpret_As_windows_1254() {}

// Main execution function
void Interpret_As_windows_1254::execute(QPlainTextEdit* editor) {
    if (!editor) {
    ...
    QString decodedText = decodeWindows1254(rawData);
    editor->setPlainText(decodedText);
    qDebug() << "[DEBUG] Windows-1254 decoding applied for file:" << filePath;
}

// Decode Windows-1254 encoded data
QString Interpret_As_windows_1254::decodeWindows1254(const QByteArray& rawData) {
    QString result;

    for (unsigned char byte : rawData) {
        auto it = windows1254Table.find(byte);
        if (it != windows1254Table.end()) {
...
const std::unordered_map<unsigned char, QChar> Interpret_As_windows_1254::windows1254Table = {
    ...
    // Add mappings for 0x84 to 0xFF, including Turkish characters
};
