...
#include "interpret_as_ibm_868.h"
#include <QFile>
#include <QDebug>

// Singleton instance
Interpret_As_IBM_868& Interpret_As_IBM_868::instance() {
    static Interpret_As_IBM_868 instance;
    return instance;
}

// EBCDIC to Unicode mapping for IBM-868 (Arabic EBCDIC)
const std::unordered_map<unsigned char, QChar> Interpret_As_IBM_868::ebcdicTable = {
...
void Interpret_As_IBM_868::execute(QPlainTextEdit* editor) {
    ...
    QString decodedText = decodeIBM868(rawData);
    editor->setPlainText(decodedText);
    qDebug() << "[DEBUG] IBM-868 Decoding applied for file:" << filePath;
}

// Decode raw data from IBM-868 encoding
QString Interpret_As_IBM_868::decodeIBM868(const QByteArray& rawData) {
    QString result;
