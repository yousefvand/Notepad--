...
#include "interpret_as_windows_1258.h"
#include <QFile>
#include <QDebug>

// Singleton instance
Interpret_As_windows_1258& Interpret_As_windows_1258::instance() {
    static Interpret_As_windows_1258 instance;
    return instance;
}

// Constructor
Interpret_As_windows_1258::Interpret_As_windows_1258() {}

// Main execution function
void Interpret_As_windows_1258::execute(QPlainTextEdit* editor) {
    if (!editor) {
    ...
    QString decodedText = decodeWindows1258(rawData);
    editor->setPlainText(decodedText);
    qDebug() << "[DEBUG] Windows-1258 decoding applied for file:" << filePath;
}

// Decode Windows-1258 encoded data
QString Interpret_As_windows_1258::decodeWindows1258(const QByteArray& rawData) {
    QString result;

    for (unsigned char byte : rawData) {
        auto it = windows1258Table.find(byte);
        if (it != windows1258Table.end()) {
...
// Complete Windows-1258 decoding table
const std::unordered_map<unsigned char, QChar> Interpret_As_windows_1258::windows1258Table = {
    ...
    {0x80, QChar(0x20AC)}, // Extend with mappings for Vietnamese characters (0x80–0xFF)
};
