...
#include "interpret_as_windows_1253.h"
#include <QFile>
#include <QDebug>

// Singleton instance
Interpret_As_windows_1253& Interpret_As_windows_1253::instance() {
    static Interpret_As_windows_1253 instance;
    return instance;
}

// Constructor
Interpret_As_windows_1253::Interpret_As_windows_1253() {}

// Main execution function
void Interpret_As_windows_1253::execute(QPlainTextEdit* editor) {
    if (!editor) {
    ...
    QString decodedText = decodeWindows1253(rawData);
    editor->setPlainText(decodedText);
    qDebug() << "[DEBUG] Windows-1253 decoding applied for file:" << filePath;
}

// Decode Windows-1253 encoded data
QString Interpret_As_windows_1253::decodeWindows1253(const QByteArray& rawData) {
    QString result;

    for (unsigned char byte : rawData) {
        auto it = windows1253Table.find(byte);
        if (it != windows1253Table.end()) {
...
const std::unordered_map<unsigned char, QChar> Interpret_As_windows_1253::windows1253Table = {
    ...
    {0x88, QChar(0x20AC)}, {0x89, QChar(0x20AC)}, {0x8A, QChar(0x20AC)}, {0x8B, QChar(0x20AC)},
    {0x8C, QChar(0x20AC)}, {0x8D, QChar(0x20AC)}, {0x8E, QChar(0x20AC)}, {0x8F, QChar(0x20AC)},
    {0x90, QChar(0x20AC)}, {0x91, QChar(0x20AC)}, {0x92, QChar(0x20AC)}, {0x93, QChar(0x20AC)},
    {0x94, QChar(0x20AC)}, {0x95, QChar(0x20AC)}, {0x96, QChar(0x20AC)}, {0x97, QChar(0x20AC)},
    // Add complete table rows
};
