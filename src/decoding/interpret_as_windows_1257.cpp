...
#include "interpret_as_windows_1257.h"
#include <QFile>
#include <QDebug>

// Singleton instance
Interpret_As_windows_1257& Interpret_As_windows_1257::instance() {
    static Interpret_As_windows_1257 instance;
    return instance;
}

// Constructor
Interpret_As_windows_1257::Interpret_As_windows_1257() {}

// Main execution function
void Interpret_As_windows_1257::execute(QPlainTextEdit* editor) {
    if (!editor) {
    ...
    QString decodedText = decodeWindows1257(rawData);
    editor->setPlainText(decodedText);
    qDebug() << "[DEBUG] Windows-1257 decoding applied for file:" << filePath;
}

// Decode Windows-1257 encoded data
QString Interpret_As_windows_1257::decodeWindows1257(const QByteArray& rawData) {
    QString result;

    for (unsigned char byte : rawData) {
        auto it = windows1257Table.find(byte);
        if (it != windows1257Table.end()) {
...
// Complete Windows-1257 decoding table
const std::unordered_map<unsigned char, QChar> Interpret_As_windows_1257::windows1257Table = {
    ...
    {0x80, QChar(0x20AC)}, // Extend for special characters (0x80–0xFF)
};
