...
#include "interpret_as_windows_1255.h"
#include <QFile>
#include <QDebug>

// Singleton instance
Interpret_As_windows_1255& Interpret_As_windows_1255::instance() {
    static Interpret_As_windows_1255 instance;
    return instance;
}

// Constructor
Interpret_As_windows_1255::Interpret_As_windows_1255() {}

// Main execution function
void Interpret_As_windows_1255::execute(QPlainTextEdit* editor) {
    if (!editor) {
    ...
    QString decodedText = decodeWindows1255(rawData);
    editor->setPlainText(decodedText);
    qDebug() << "[DEBUG] Windows-1255 decoding applied for file:" << filePath;
}

// Decode Windows-1255 encoded data
QString Interpret_As_windows_1255::decodeWindows1255(const QByteArray& rawData) {
    QString result;

    for (unsigned char byte : rawData) {
        auto it = windows1255Table.find(byte);
        if (it != windows1255Table.end()) {
...
// Complete Windows-1255 decoding table
const std::unordered_map<unsigned char, QChar> Interpret_As_windows_1255::windows1255Table = {
    ...
    {0x88, QChar(0x20AC)}, {0x89, QChar(0x20AC)}, {0x8A, QChar(0x20AC)}, {0x8C, QChar(0x20AC)},
    };
